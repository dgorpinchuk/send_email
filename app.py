...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        form.addWidget(QLabel("Sender name"), 0, 0)
        form.addWidget(self.sender_name, 0, 1, 1, 4)
        form.addWidget(QLabel("Subject"), 1, 0)
        form.addWidget(self.subject, 1, 1, 1, 4)
        form.addWidget(QLabel("HTML template"), 2, 0)
        form.addWidget(self.html_path, 2, 1, 1, 2)
        form.addWidget(choose_html, 2, 3)
        form.addWidget(edit_html, 2, 4)
        ...
        choose_data = QPushButton("Choose…")
        choose_data.clicked.connect(self.choose_data)
        self.dedupe = QCheckBox("Remove duplicates")
        self.dedupe.setChecked(True)
        self.dedupe.stateChanged.connect(lambda: self._read_data() if self.data_path.text() else None)
        data.addWidget(QLabel("Database"), 0, 0)
        data.addWidget(self.data_path, 0, 1, 1, 2)
        data.addWidget(choose_data, 0, 3)
        data.addWidget(self.dedupe, 0, 4)
        ...
        self.test_address.setPlaceholderText("Test recipient")
        ...
        actions.addWidget(self.test_address)
        ...
        progress.addWidget(self.current)
        progress.addWidget(self.log)
        ...
        about.triggered.connect(
            lambda: QMessageBox.information(
                self, "SMTP Mail Sender", "PySide6 email campaign tool"
            )
        )
    ...
    def _profile_changed(self, _index: int):
        # Profile details intentionally live only inside the Edit dialog now.
        # Selecting a profile therefore only changes the active profile.
    ...
    def choose_data(self):
        path, _ = QFileDialog.getOpenFileName(
            self,
            "Choose recipient database",
            "",
            "Recipient files (*.txt *.csv *.xlsx);;All files (*)",
        )
        if path:
    ...
    def _read_data(self):
        try:
            self.rows, self.columns = load(
                self.data_path.text(), self.dedupe.isChecked()
            )
            self._update_stats()
        except Exception as exc:
    ...
    def _ensure_ready(self, test=False):
        ...
        if not self.template_text:
            raise ValueError("Choose an HTML template")

        profile_name = self.profile_combo.currentText().strip()
        if not profile_name:
            raise ValueError("Choose an SMTP profile")
        smtp = profiles.get(profile_name)
        if (
            not str(smtp.get("username", "")).strip()
            or not smtp.get("password")
            or not str(smtp.get("server", "")).strip()
        ):
        ...
        if not self.rows:
            self._read_data()
        if not self.rows:
            raise ValueError("Choose a recipient database")
        missing = validate_columns(self.template_text, self.columns)
        if missing:
            raise ValueError("Missing columns: " + ", ".join(missing))
        valid, invalid = validate(self.rows)
        if invalid:
            raise ValueError(f"There are {len(invalid)} invalid recipient rows")
        if not valid:
    ...
    def preview(self):
        try:
            ...
            layout.addWidget(
                QLabel(f"Subject: {self.subject.text()}\nTo: {self.rows[0]['email']}")
            )
    ...
    def send_test(self):
        try:
            ...
            if not address:
                raise ValueError("Enter a test recipient")
            row = self.rows[0] if self.rows else {"email": address}
            body = render(self.template_text, row)
            ok, error = send_email(
                str(smtp["server"]).strip(),
                int(smtp["port"]),
                str(smtp["username"]).strip(),
                smtp["password"],
                str(smtp["username"]).strip(),
                address,
            ...
            if ok:
                QMessageBox.information(
                    self, "Test sent", f"Test email sent to {address}."
                )
            else:
